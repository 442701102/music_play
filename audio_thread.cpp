...
static int audio_parameter_settings(void *MusicsState)
{
    ddwork_s *d_pstMusicsState= (ddwork_s *) MusicsState;
    ...
    /** Define an AVFormatContext pointer to point to media information for obtaining audio streams **/
       ...
       /**  检查输入的 声道数 和 声道布局 **/
       if (codecCtx->channels > 0 && codecCtx->channel_layout == 0) //有声道数没有声道布局，所以要设置声道布局
       {
            codecCtx->channel_layout = av_get_default_channel_layout(codecCtx->channels);//设置声道布局
       }
       else if (codecCtx->channels == 0 && codecCtx->channel_layout > 0) //有声道布局没有声道数，所以要设置声道数
       {
            codecCtx->channels = av_get_channel_layout_nb_channels(codecCtx->channel_layout);
       }
       else if (codecCtx->channels == 0 && codecCtx->channel_layout == 0)
       {
            cout <<__FUNCTION__<< "[channels and channel_layout error] " <<endl;
            codecCtx->channel_layout = av_get_default_channel_layout(codecCtx->channels);//设置声道布局
            codecCtx->channels = av_get_channel_layout_nb_channels(codecCtx->channel_layout);
       }
       ...
       int out_nb_samples=codecCtx->frame_size;	 //nb_samples: AAC-1024 MP3-1152  格式大小
       int out_sample_rate = codecCtx->sample_rate;//采样率	pCodecCtx->sample_rate 44100  48000
       int out_channels = av_get_channel_layout_nb_channels(out_channel_layout);	 //根据声道格式返回声道个数

       /*有的音频格式(WAV/FLAC等)数据头为非标准格式，从frame_size中得不到正确的数据大小，只能解码一帧数据后才可以获得*/
       if(out_nb_samples== 0 ) out_nb_samples = SDL_AUDIO_BUFFER_SIZE;


       d_pstMusicsState->out_buffer_size = av_samples_get_buffer_size(NULL,out_channels,out_nb_samples,out_sample_fmt,1);//获取输出缓冲大小

       cout <<__FUNCTION__<< "[codecCtx->frame_size] " <<codecCtx->frame_size<<endl;
       cout <<__FUNCTION__<< "[codecCtx->sample_rate] " <<codecCtx->sample_rate<<endl;
...
int music_thread(void *arg)
{
    ...
    audio_parameter_settings(d_pstMusicsState);
    AVCodecContext  *codecCtx       = d_pstMusicsState->p_pstACodecCtx;
...
//d_pstMusicsState->audio_frame->format -> float 4bit
 //          printf("diff-1 sample_(%d-%d)\n",d_pstMusicsState->audio_frame->nb_samples,codecCtx->frame_size);
           //数据格式转换

           swr_convert(d_pstMusicsState->swr_ctx,&d_pstMusicsState->audio_buf,d_pstMusicsState->audio_frame->nb_samples,(const uint8_t**)d_pstMusicsState->audio_frame->data,d_pstMusicsState->audio_frame->nb_samples);

           d_pstMusicsState->audio_buf_index++;
           d_pstMusicsState->audio_buf_size +=codecCtx->frame_size;
       }
  ...
  // qDebug() << QString("[music-info]index:%1,pts:%2 ,packet size:%3,time %4").arg(d_pstMusicsState->audio_buf_index).arg(packet->pts).arg(packet->size).arg(d_pstMusicsState->audio_clock)<< endl;
     ...
     //         else
          ...
          //out_buffer_size
          //音频长度
          d_pstMusicsState->audio_pkt_size =d_pstMusicsState->out_buffer_size;
          //当前播放位置
