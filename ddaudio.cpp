...
#endif
//3. get the index position of the audio stream,
   l_s32AStreamSubscript = get_streamsscript(l_pstFormatCtx,AVMEDIA_TYPE_AUDIO);//Audio stream sequence number
  /** If the Audio stream is not found, process **/
...
//4.Open the audio decoder  Codec type or id mismatches
